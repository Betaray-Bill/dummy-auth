  ...
  if(!refresh_token){
    ...
    jwt.verify(refresh_token, process.env.JWT_SECRET_KEY, (err, decoded) => {
      if(err){
        ...
        exist = true;
        const access_token = jwt.sign({email: decoded.email}, process.env.JWT_SECRET_KEY_, {expiresIn: '1m'});
        res.cookie('access_token', access_token, {expires: 2592000, httpOnly: true});
        return true;
      }
    })
  }
